...
def build_native_openai_messages(
    ...
    if request.system:
        if is_reasoning_model:
            _messages.append(
                ChatCompletionDeveloperMessageParam(role="developer", content=request.system)
            )
        else:
            _messages.append(
    ...
    if has_images:
            ...
            if isinstance(part, TextPart):
                ...
                if part.uri:
                    content_parts.append(
                        ChatCompletionContentPartImageParam(
                            type="image_url",
                            image_url=ImageURL(url=part.uri),
                        )
                    )
                elif part.data_b64:
        ...
        _messages.append(ChatCompletionUserMessageParam(role="user", content=content_parts))
    else:
        # Fallback to pure string processing if no images
        _messages.append(
            ChatCompletionUserMessageParam(role="user", content=request.to_text_prompt())
        )
