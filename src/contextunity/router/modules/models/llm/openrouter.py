...
from .types import OpenRouterProviderConfig
...
class OpenRouterLLM(BaseModel):
    ...
    def __init__(
        ...
        """Create an ``AsyncOpenAI`` client targeting the OpenRouter multi-model gateway."""
        try:
            from openai import AsyncOpenAI
        except ImportError as e:
            raise ConfigurationError(
                "OpenRouterLLM requires `openai` package. Install with `pip install openai`."
            ) from e

        resolved_name = (model_name or "").strip() or "openai/gpt-5.1"
        super().__init__(provider="openrouter", model_name=resolved_name)
        ...
        self._client: AsyncOpenAI = AsyncOpenAI(
            api_key=(config.openrouter.api_key or "skip"),
            base_url=self._base_url,
            max_retries=config.llm.max_retries,
        )
        if kwargs:
            logger.debug("Ignoring unsupported OpenRouterLLM kwargs: %s", sorted(kwargs.keys()))
    ...
    def capabilities(self) -> ModelCapabilities:
        """Declare modality support for the OpenRouter backend."""
        return self._capabilities

    @override
    async def _generate(self, request: ModelRequest) -> ModelResponse:
        """Call the OpenAI-compatible OpenRouter endpoint and return a complete response."""
        messages = build_native_openai_messages(request)
        pc = OpenRouterProviderConfig.model_validate(request.provider_config)
    ...
    def _stream(
        ...
        """Stream token deltas from the OpenAI-compatible OpenRouter endpoint."""
        pc = OpenRouterProviderConfig.model_validate(request.provider_config)
        messages = build_native_openai_messages(request)
        json_mode = resolve_json_object_mode(
            request_response_format=request.response_format,
        ...
        async def _event_stream() -> AsyncIterator[ModelStreamEvent]:
            """Yield ``TextDelta`` / ``UsageEvent`` from the OpenAI-compatible OpenRouter endpoint stream."""
            full = ""
