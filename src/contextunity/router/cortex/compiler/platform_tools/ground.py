...
from contextunity.router.cortex.compiler.platform_tools.prompts import RAG_SYSTEM_PROMPT
...
from contextunity.router.modules.providers.storage.vertex_grounding import generate_with_grounding
...
async def generate_with_native_grounding(state: GraphState) -> StateUpdate:
    ...
    rag_system_prompt_override = str(dyn.get("rag_system_prompt_override") or "")
    ...
    graph_facts_raw: object = state.get("graph_facts") or []
    graph_facts = (
        [fact for fact in graph_facts_raw if isinstance(fact, str)]
    ...
    try:
        ...
        if rag_filter and len(citations) == 0:
            ...
            from contextunity.router.cortex.compiler.platform_tools.no_results import (
                no_results_response,
            )

            # Format conversation history (excl. current query) via shared utility
            ...
            response_content = extract_message_content(no_results_msg)
        ...
        for cit in citations:
            if not is_object_dict(cit):
                continue
            title_raw = cit.get("title", "")
            uri_raw = cit.get("uri", "")
            url_raw = cit.get("url", "")
            title = title_raw if isinstance(title_raw, str) else ""
            url = (
                uri_raw
                if isinstance(uri_raw, str) and uri_raw
                else (url_raw if isinstance(url_raw, str) and url_raw else "")
            )
