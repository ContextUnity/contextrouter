...
def test_retrieval_pipeline_calls_vertex_and_builds_citations(monkeypatch) -> None:
    from contextunity.router.modules.retrieval.pipeline import BaseRetrievalPipeline
    ...
    async def mock_execute(
        ...
        from contextunity.router.modules.retrieval.pipeline import PipelineResult
    ...
    monkeypatch.setattr(
        "contextunity.router.modules.retrieval.rag.pipeline.get_graph_facts",
        lambda _state: ["f1"],
    )
    monkeypatch.setattr(
        "contextunity.router.modules.retrieval.rag.pipeline.get_reranker",
        lambda **kwargs: MockReranker(),
    )
    monkeypatch.setattr(
        "contextunity.router.modules.retrieval.rag.pipeline.build_citations",
        lambda docs, **_kw: (
